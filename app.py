...
libs.say_audio("Welcome to Irisya. The active profil is "+active_profil_name, \
active_profil['language'], active_profil['slowly'])

libs.say_audio("I am ready to help you. Say 'ok Irisia' to talk with me.", \
active_profil['language'], active_profil['slowly'])
...
while True:
    ...
    try:
        ...
        if libs.check_triggers(user_said.upper(), active_profil['triggers']):
            ...
            history = [{"role": "user", "content": active_profil['messages']\
            [platform.system()].format(request=prompt)}]
            ...
            if libs.process_request(result) == "_":
                print('[LOG] Starting a process... ({process})'.format(\
                process=result.split('#')[-1]))
